...
/**
 * @ClassName: ForgetPassActivity
 * @Description:忘记密码
 * @author zz
 * @date 2015年1月15日 下午12:31:01
 * @version 1
 * @company 西安甲骨企业文化传播有限公司
 */
public class ForgetPassActivity extends VerificationCodeActivity implements
	...
	private void findView() {
		mAccountEt = (EditText) findViewById(R.id.forgetpass_user_name_et);
		mPhoneEt = (EditText) findViewById(R.id.forgetpass_user_phone_et);
		mCodeEt = (EditText) findViewById(R.id.forgetpass_user_code_et);
		mNewPassEt = (EditText) findViewById(R.id.forgetpass_user_pass_et);
		mRePassEt = (EditText) findViewById(R.id.forgetpass_user_repass_et);
		...
		mOkBtn = (Button) findViewById(R.id.forgetpass_ok_btn);
		mCancelBtn = (Button) findViewById(R.id.forgetpass_cancel_btn);
		mOkBtn.setOnClickListener(this);
		mCancelBtn.setOnClickListener(this);
		mAccountTv = (TextView) findViewById(R.id.forgetpass_user_name_tv);
		mLogoIv = (ImageView) findViewById(R.id.forgetpass_user_logo_img);
		super.setOnCodeBtnClickListener(this);
	}

	private void sendRequest(String account, String phone, String code,
			String pass) {
		showProgress();

		String url = ScenicApplication.SEVER_PATH
				+ "scenicUser/forgectPass.htm";
		RequestParams params = new RequestParams();
		params.put("user", account);
		params.put("phone", phone);
		params.put("code", code);
		try {
			params.put("newpass", Md5.En(pass));
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.forgetpass_ok_btn:
			String account = mAccountEt.getText().toString();
			String phone = mPhoneEt.getText().toString();
			String code = mCodeEt.getText().toString();
			String newpass = mNewPassEt.getText().toString();
			String repass = mRePassEt.getText().toString();
			if (validateInput(account, phone, code, newpass, repass)) {
				sendRequest(account, phone, code, repass);
			}
			break;
		case R.id.forgetpass_cancel_btn:
			onBackPressed();
			break;
		default:
			break;
	...
	private boolean validateInput(String account, String phone, String code,
